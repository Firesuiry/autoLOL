...
class Equip:
	...
	def __init__(self,id:int,parent=None):
		...
		for i in EquipList:
			if i[0] == id:
				self.name = i[1]
				self.ChildEquip = [Equip(c,parent=self) for c in i[3]]
				self.NeedMoney = i[2]
				return
	@property
	def Money(self) -> int:
		...
		if NokoruMoney >= self.NeedMoney:
			...
			for i in self.ChildEquip:
				if i.Flag:
					Q_W += i.NeedMoney
					continue
				if NokoruMoney >= i.NeedMoney:
					P_W = 0
					for w in i.ChildEquip:
						if w.Flag:
							P_W += w.NeedMoney
					NokoruMoney -= i.NeedMoney - P_W
					i.Flag = True
				else:
					M_W = 0
					for j in i.ChildEquip:
						if j.Flag:
							M_W += j.NeedMoney
							continue
						if NokoruMoney >= j.NeedMoney:
							NokoruMoney -= j.NeedMoney
							j.Flag = True
					if NokoruMoney >= i.NeedMoney - M_W:
						NokoruMoney -= i.NeedMoney - M_W
						i.Flag = True
					M_W = 0
					for j in i.ChildEquip:
						if j.Flag:
					...
					if NokoruMoney >= i.NeedMoney - M_W:
						NokoruMoney -= i.NeedMoney - M_W
						i.Flag = True
			if NokoruMoney >= self.NeedMoney - Q_W:
